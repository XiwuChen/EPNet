...
def model_joint_fn_dist_decorator():
    ModelReturn = namedtuple("ModelReturn", ['loss', 'tb_dict', 'disp_dict'])
    MEAN_SIZE = torch.from_numpy(cfg.CLS_MEAN_SIZE[0]).cuda()
    ...
    def get_rpn_loss(model, rpn_cls, rpn_reg, rpn_cls_label, rpn_reg_label, tb_dict):
        ...
        loss_loc, loss_angle, loss_size, loss_iou, reg_loss_dict = \
            loss_utils.get_reg_loss_dist(torch.sigmoid(rpn_cls_flat), None,
                                         rpn_reg.view(point_num, -1),
    ...
    def get_rcnn_loss(model, ret_dict, tb_dict):
        ...
        loss_loc, loss_angle, loss_size, loss_iou, reg_loss_dict = \
            loss_utils.get_reg_loss_dist(torch.sigmoid(rcnn_cls_flat), None,
                                         rcnn_reg.view(batch_size, -1),
                                         ...
                                         gt_iou_weight=gt_iou_weight[fg_mask],
                                         use_iou_branch=cfg.USE_IOU_BRANCH,
                                         iou_branch_pred=iou_branch_pred)
