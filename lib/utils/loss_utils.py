...
def mask_weight_sum(loss,mask_weight):
    """
    """
    if len(loss.shape)==2:
        loss = (loss.mean(-1)*mask_weight).sum()
    elif len(loss.shape) == 1:
        loss = (loss * mask_weight).sum()
    else:
        raise NotImplementedError("")
    return loss

def get_reg_loss_dist(cls_score, mask_score, pred_reg, reg_label, loc_scope, loc_bin_size, num_head_bin, anchor_size,fg_mask,
                 get_xz_fine = True, get_y_by_bin = False, loc_y_scope = 0.5, loc_y_bin_size = 0.25,
                 get_ry_fine = False,
                 use_cls_score = False, use_mask_score = False,
                 gt_iou_weight = None,
    ...
    """
    fg_sum = fg_mask.sum()
    mask_weight =  fg_mask.float()/max(fg_sum,1.)
    mask_weight = mask_weight.detach()
    per_loc_bin_num = int(loc_scope / loc_bin_size) * 2
    loc_y_bin_num = int(loc_y_scope / loc_y_bin_size) * 2
    ...
    loss_x_bin = F.cross_entropy(pred_reg[:, x_bin_l: x_bin_r], x_bin_label,reduction='none')
    loss_z_bin = F.cross_entropy(pred_reg[:, z_bin_l: z_bin_r], z_bin_label,reduction='none')
    loss_x_bin = mask_weight_sum(loss_x_bin,mask_weight)
    loss_z_bin = mask_weight_sum(loss_z_bin,mask_weight)
    reg_loss_dict['loss_x_bin'] = loss_x_bin.item()
    reg_loss_dict['loss_z_bin'] = loss_z_bin.item()
    loc_loss += loss_x_bin + loss_z_bin
    ...
    if get_xz_fine:
        ...
        loss_x_res = F.smooth_l1_loss((pred_reg[:, x_res_l: x_res_r] * x_bin_onehot).sum(dim = 1), x_res_norm_label,reduction='none')
        loss_z_res = F.smooth_l1_loss((pred_reg[:, z_res_l: z_res_r] * z_bin_onehot).sum(dim = 1), z_res_norm_label,reduction='none')
        loss_x_res = mask_weight_sum(loss_x_res,mask_weight)
        loss_z_res = mask_weight_sum(loss_z_res,mask_weight)
        reg_loss_dict['loss_x_res'] = loss_x_res.item()
        reg_loss_dict['loss_z_res'] = loss_z_res.item()
        loc_loss += loss_x_res + loss_z_res
    ...
    if get_y_by_bin:
        ...
        loss_y_bin = F.cross_entropy(pred_reg[:, y_bin_l: y_bin_r], y_bin_label,reduction='none')
        loss_y_bin = mask_weight_sum(loss_y_bin,mask_weight)
        loss_y_res = F.smooth_l1_loss((pred_reg[:, y_res_l: y_res_r] * y_bin_onehot).sum(dim = 1), y_res_norm_label)
        loss_y_res= mask_weight_sum(loss_y_res,mask_weight)
        ...
        loss_y_offset = F.smooth_l1_loss(pred_reg[:, y_offset_l: y_offset_r].sum(dim = 1), y_offset_label,reduction='none')
        loss_y_offset = mask_weight_sum(loss_y_offset,mask_weight)
        reg_loss_dict['loss_y_offset'] = loss_y_offset.item()
        loc_loss += loss_y_offset
    ...
    ry_bin_onehot = torch.cuda.FloatTensor(ry_bin_label.size(0), num_head_bin).zero_()
    ry_bin_onehot.scatter_(1, ry_bin_label.view(-1, 1).long(), 1)
    loss_ry_bin = F.cross_entropy(pred_reg[:, ry_bin_l:ry_bin_r], ry_bin_label,reduction='none')
    loss_ry_bin = mask_weight_sum(loss_ry_bin,mask_weight)
    loss_ry_res = F.smooth_l1_loss((pred_reg[:, ry_res_l: ry_res_r] * ry_bin_onehot).sum(dim = 1), ry_res_norm_label,reduction='none')
    loss_ry_res = mask_weight_sum(loss_ry_res,mask_weight)
    ...
    size_res_norm_label = (reg_label[:, 3:6] - anchor_size) / anchor_size
    size_res_norm = pred_reg[:, size_res_l:size_res_r]
    size_loss = F.smooth_l1_loss(size_res_norm, size_res_norm_label,reduction='none')
    size_loss= mask_weight_sum(size_loss,mask_weight)
    ...
    if use_iou_branch:
        ...
        iou_branch_loss = -(iou_tmp_taget.detach() * torch.log(iou_branch_pred_flat) + (
                    1 - iou_tmp_taget.detach()) * torch.log(1 - iou_branch_pred_flat))
        reg_loss_dict['iou_branch_loss'] = mask_weight_sum(iou_branch_loss,mask_weight)
    ...
    iou_tmp = torch.max(iou_tmp, iou_tmp.new().resize_(iou_tmp.shape).fill_(1e-4))
    iou_loss = -torch.log(iou_tmp)
    iou_loss = mask_weight_sum(iou_loss,mask_weight)
